	...
	#          following keys:
	#            n_images - number of images x
	#            n_dogs_img - number of dog images
	#            n_notdogs_img - number of NON-dog images
	#            n_match - number of matches between pet & classifier labels x
	#            n_correct_dogs - number of correctly classified dog images x
	#            n_correct_notdogs - number of correctly classified NON-dog images x
...
def calculates_results_stats(results_dic):
	...
	for key in results_dic:
		...
		# checks if there is match between predicted and actual
		if labels_match:
			results_stats_dic['n_match'] += 1
		
		# checks if image is ACTUALLY a dog
		if results_dic[key][3] == 1:
			results_stats_dic['n_dogs_img'] += 1
			
			if results_dic[key][4] == 1: # check if classified guess is dog
				results_stats_dic['n_correct_dogs'] += 1

			if labels_match: # if the breed was guessed correctly
				results_stats_dic['n_correct_breed'] += 1
		else:
			# image is ACTUALLY not a dog
			results_stats_dic['n_notdogs_img'] += 1

			if results_dic[key][4] == 0: # if classification isn't a dog
				results_stats_dic['n_correct_notdogs'] += 1

	# calculating the percentages of the stats
